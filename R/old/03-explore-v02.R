...
save_viz <-
  function(viz,
           ...
           # filename_save = rlang::quo_name(viz)
           w = 7,
           h = 7,
           overwrite = TRUE,
           create_backup = FALSE,
    ...
    if (save == TRUE) {
      # filename_save <- deparse(substitute(viz))
      filepath_save <-
        str_c(dir_figs, filename_save, filename_figs_ext)
      # ggsave(path = filepath_save, plot = viz, units = "in", width = w, height = h)
      ggplot2::ggsave(
        viz,
...
save_viz(viz_cnt_bycomplvl_grid, save = TRUE)
...
grid.arrange(viz_cnt_comp__bycompbyx_grid)
save_viz(viz_cnt_comp__bycompbyx_grid)
#'
...
viz_cnt_byschool_bycomplvl_byconf
save_viz(viz_cnt_byschool_bycomplvl_byconf)
#'
...
viz_comp_stats <- viz_comp_stats # %>% add_common_viz_comp_stats_elements()

viz_comp_stats
save_viz(viz_comp_stat)
...
viz_comp_stats_byear <-
  ...
  geom_point(aes(x = year, y = mean), fill = color_neutral_1, size = 2) +
  coord_flip() +
  labs(subtitle = "By Year")
viz_comp_stats_byear
# viz_comp_stats_byear <- viz_comp_stats_byear %>% add_common_viz_comp_stats_elements()
viz_comp_stats_byear <-
  viz_comp_stats_byear +
  guides(fill = FALSE) +
  scale_x_continuous(limits = c(q05_min, q95_max)) +
