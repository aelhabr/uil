...
filepath_download <- str_c(dir_download, filename_download, filename_download_ext)
...
clean_schools <- function(raw, complvl) {
  ...
  if(complvl == "district") {
    renamed <- renamed %>% rename(school_city_complvl = school_city_district)
  } else if (complvl == "regional") {
    renamed <- renamed %>% rename(school_city_complvl = school_city_region)
  } else if (complvl == "state") {
    renamed <- renamed %>% rename(school_city_complvl = school_city)
  } else {
    stop()
  }
  ...
  edited <-
    renamed %>%
    separate(school_city_complvl, c("school", "city_complvl"), sep = ", ")

  if(complvl == "state") {
    edited <-
      edited %>% mutate(city_complvl = str_c(city_complvl, " (0)"))
  }

  output <-
    edited %>%
    separate(city_complvl, c("city", "complvl"), sep = "\\s\\(") %>%
    mutate(complvl = str_replace(complvl, "\\)", "")) %>%
    mutate(complvl = as.numeric(complvl)) %>%
    rename(complvl_num = complvl) %>%
    mutate(school = str_replace(school, "\\s[Hh].*[Ss]", ""))
}
#'
#'
#'
  ...
  while(j <= length(confs_scrape)) { # {}
    ...
    while(k <= length(complvls_scrape)) { # {}
      ...
      while(l <= length(comps_scrape)) { # {}
        ...
        url <- str_c(url_base, year, "/", conf, "/", complvl, "/", comp, url_suffix)
        ...
        schools_mutated <-
          schools_cleaned %>%
          mutate(year = year, conf = conf, complvl = complvl, comp = comp, url = url)
...
if (export == TRUE) {
  write_csv(schools_all, filepath_export_schools)
  write_csv(persons_all, filepath_export_persons)

  if(create_backup == TRUE) {
    write_csv(schools_all, filepath_export_schools_ts)
    write_csv(persons_all, filepath_export_persons_ts)
  }
}
